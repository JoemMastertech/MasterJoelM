...
class SidebarManager {
    constructor() {
        ...
        this.config = {
            ...
            desktopBreakpoint: 1280, // Updated to match 'lg' breakpoint (was 1024)
            openAttribute: 'data-sidebar-state',
            openValue: 'open'
        };

        // Track mode for breakpoint changes
        this.isMobileMode = window.innerWidth < this.config.desktopBreakpoint;

        this._initOverlay();
        this._bindGlobalEvents();

        // Breakpoint Listener (NEW)
    ...
    /**
     * Handles window resize events to manage sidebar state transitions
     */
    handleBreakpointChange() {
        const wasMobileMode = this.isMobileMode;
        this.isMobileMode = window.innerWidth < this.config.desktopBreakpoint;

        // If transitioning TO/FROM Desktop:
        if (wasMobileMode !== this.isMobileMode) {
            if (!this.isMobileMode) {
                // Entering Desktop: Remove Dropback hacks
                if (this.appContainer) this.appContainer.removeAttribute('data-backdrop');
            } else {
                // Entering Mobile/Tablet: Close sidebar if open to prevent weird overlay state
                this.closeAll();
            }
        }
    }
    ...
    _updateEnvironment(isOpen, sidebarId) {
        ...
        if (isOpen) {
            ...
            if (isDesktop && (sidebarId === 'order-sidebar' || sidebarId === 'side-panel')) { // Legacy alias
